 ...
 * Vinculado al Cluster 2: drone_dt (Inventario, Telemetría y Logística)
 */
const productSchema = new mongoose.Schema(
    {
        ...
        // Referencia cruzada (Se guarda el ID, pero reside físicamente en otro Cluster)
        user: {
...
// --- MIDDLEWARES (Lógica de Negocio Automática) ---
productSchema.pre('save', function(next) {
    if (!this.isModified('name')) return next();
    this.slug = slugify(this.name, { lower: true, strict: true });
    
    // Auto-poblar imageUrl si no existe pero hay imágenes en el array
...
// --- ÍNDICES (Optimización de Búsqueda) ---
productSchema.index({ name: 'text', brand: 'text', description: 'text' });
productSchema.index({ currentLocation: '2dsphere' });
productSchema.index({ slug: 1 });
productSchema.index({ category: 1 });

// --- VIRTUALS (Props calculadas en tiempo real) ---
productSchema.virtual('isReadyForFlight').get(function() {
    return this.status === 'disponible' && this.stock > 0;
});

/**
 * EXPORTACIÓN MULTI-CLUSTER BLINDADA
 * 1. Previene el error 'Cannot overwrite model once compiled'.
 * 2. Vincula forzosamente el modelo a assetsConnection (Cluster 2).
 */
