    ...
    <section className="py-12 bg-gray-100">
      <div className="container mx-auto px-4 max-w-[1900px] min-w-[310px]">
        ...
        <div className="relative flex flex-col items-center">
          ...
          <div className="hidden sm:grid sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 mt-6">
            {videos.map((video, index) => (
              <img
                key={video.id}
                src={video.thumbnail}
                alt={video.title}
                className={`w-full h-32 object-cover rounded cursor-pointer ${index === currentVideo ? 'border-2 border-blue-600' : ''}`}
                onClick={() => setCurrentVideo(index)}
              />
            ))}
          </div>
