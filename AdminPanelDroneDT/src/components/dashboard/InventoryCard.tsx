...
// Configuración de estados en español para claridad operativa
const STATUS_MAP: Record<ProductStatus, { color: string; bg: string; label: string }> = {
  AVAILABLE: { color: 'text-emerald-500', bg: 'bg-emerald-500/10', label: 'DISPONIBLE' },
  SOLD: { color: 'text-red-500', bg: 'bg-red-500/10', label: 'VENDIDO' },
  RESERVED: { color: 'text-amber-500', bg: 'bg-amber-500/10', label: 'RESERVADO' },
  DRAFT: { color: 'text-slate-500', bg: 'bg-slate-500/10', label: 'BORRADOR' },
};

export const InventoryCard = memo(({ product }: Props) => {
  // Safe Guard: Error técnico en español
  if (!product?.sku) {
    return (
      <div className="border border-red-500/20 bg-red-500/5 p-3 flex items-center gap-2">
        <AlertTriangle className="h-3 w-3 text-red-500" />
        <span className="font-mono text-[9px] text-red-500 uppercase">ERROR_DATOS_L5</span>
      </div>
    );
  }
  ...
  return (
    ...
    >
      ...
      {/* Matrix Data: Especificaciones en Español */}
      <div className="z-10 grid grid-cols-2 gap-2 border-y border-white/5 py-2 my-1">
        <div className="flex flex-col border-r border-white/5">
          ...
          <span className="text-[7px] uppercase tracking-[0.1em] text-white/20 mt-1 font-bold">Peso_Quilates</span>
        </div>
        ...
        <div className="flex flex-col pl-2">
          ...
          <span className="text-[7px] uppercase tracking-[0.1em] text-white/20 mt-1 font-bold">Valor_USD</span>
        </div>
      </div>

      {/* Footer: Sincronización y Acción */}
      <div className="z-10 flex items-center justify-between">
        <div className="flex items-center gap-1.5 px-1.5 py-0.5 bg-white/[0.03] rounded-sm">
          ...
          <span className="font-mono text-[8px] text-white/40 uppercase">
            SINC: {new Date(product.lastUpdate).toLocaleTimeString('es-CO', { hour12: false })}
          </span>
        </div>
