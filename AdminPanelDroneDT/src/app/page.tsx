...
// CORRECCIÓN DIRECTA: Importación por defecto desde la ubicación real de tu Logo
          ...
          >
            <div className="w-full max-w-md bg-zinc-950/40 border border-white/5 rounded-2xl p-8 backdrop-blur-xl relative overflow-hidden shadow-[0_0_50px_rgba(0,0,0,0.9)]">
              ...
              <button
                ...
                className="w-full flex items-center justify-center gap-3 bg-[#FFD700] text-black h-12 rounded-xl font-black text-[11px] tracking-[0.25em] uppercase hover:bg-yellow-400 transition-all active:scale-[0.98] shadow-[0_0_30px_rgba(223,190,0,0.15)] group"
              >
        ...
        {currentFlow === 'DASHBOARD' && (
          ...
          >
            ...
            <div className="flex-1 flex flex-col overflow-hidden h-full">
              ...
              <header className="flex items-center justify-between fluid-header border-b border-white/5 bg-black/40 backdrop-blur-md">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-[9px] text-zinc-500 tracking-[0.3em] uppercase mt-1">
                      Nieto_Laboratory // Production_Environment
                    </p>
                  </div>
                </div>
                
                <div className="flex items-center gap-4 text-[10px] text-zinc-400 bg-white/[0.02] border border-white/5 px-4 py-2 rounded-xl">
                  <span className="flex items-center gap-2">
                    <span className="w-1.5 h-1.5 bg-emerald-500 rounded-full animate-pulse" />
                    SYS_TIME: <span className="text-white font-bold">{systemTime}</span>
                  </span>
                </div>
              </header>

              {/* HUD DE MÉTRICAS CORE: PRODUCTOS, DISPONIBLES Y VENDIDOS */}
              <section className="fluid-hud grid grid-cols-3 gap-4 bg-zinc-950/40 border-b border-white/5 font-mono">
                <div className="bg-white/[0.01] border border-white/5 p-3 rounded-xl flex flex-col justify-between">
                  ...
                  <div className="fluid-text-metric font-black text-[#FFD700] tracking-tight mt-1">
                    {productMetrics.totalDisponibles} <span className="text-[10px] font-normal text-zinc-500">U</span>
                  </div>
                </div>
                <div className="bg-white/[0.01] border border-white/5 p-3 rounded-xl flex flex-col justify-between">
                  <div className="flex items-center gap-2 text-zinc-500 text-[9px] tracking-widest uppercase">
                  ...
                  <div className="fluid-text-metric font-black text-emerald-400 tracking-tight mt-1">
                    {productMetrics.totalVendidas} <span className="text-[10px] font-normal text-zinc-500">QTY</span>
                  </div>
                </div>
                <div className="bg-white/[0.01] border border-white/5 p-3 rounded-xl flex flex-col justify-between">
                  <div className="flex items-center gap-2 text-zinc-500 text-[9px] tracking-widest uppercase">
                  ...
                  <div className="fluid-text-metric font-black text-white tracking-tight mt-1">
                    {productMetrics.eficienciaFlota}
                  </div>
                </div>
              </section>

              {/* RETÍCULA BENTO RESTRINGIDA (310px a 1900px sin scrollbar) */}
              <main className="flex-1 fluid-main overflow-hidden grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 auto-rows-fr h-full w-full">
                
                {/* BLOQUE DE VENTAS / CENTRAL DE FLUJO */}
                ...
                <div className="bg-zinc-950/20 border border-white/5 rounded-xl p-4 flex flex-col gap-3 justify-between overflow-hidden">
                  {isSystemChecking ? (
                    <DroneSkeleton />
                  ) : (
                    <ErrorShield />
                ...
                {/* MÓDULO CORE DE INVENTARIO DIGITAL TWIN */}
                <div className="lg:col-span-2 bg-zinc-950/20 border border-white/5 rounded-xl p-2 overflow-hidden flex flex-col justify-between">
                  <Inventory />
                </div>
