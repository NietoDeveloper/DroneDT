...
export default function DashboardPage() {
  ...
  // 2. Manejador de Autenticación de Operador (Acceso instantáneo para pruebas de flujo)
  const handleUplinkAuth = (e: React.FormEvent) => {
    e.preventDefault();
    // Avanza directamente al dashboard sin requerir validación de Backend por ahora
  ...
  // =========================================================================
  // VISTA B: LOGIN DE ACCESO MÁXIMA SEGURIDAD (Esquema Gold / Ámbar Industrial)
  // =========================================================================
        ...
        <div className="relative w-full max-w-[400px] border border-white/10 bg-zinc-950/50 p-6 lg:p-8 backdrop-blur-md">
          ...
          <form onSubmit={handleUplinkAuth} className="space-y-4">
            <div>
              ...
              <div className="relative flex items-center bg-black border border-white/10 focus-within:border-amber-500/50 transition-all">
                <input
                  ...
                  className="w-full bg-transparent text-xs p-3 outline-none text-amber-400 uppercase placeholder:text-zinc-800 font-bold"
                />
                <button
                  type="button"
            ...
            <button
              type="submit"
              disabled={isAuthenticating}
              className="w-full h-11 bg-amber-500 hover:bg-amber-400 hover:shadow-[0_0_15px_rgba(245,158,11,0.4)] text-black font-black text-xs uppercase tracking-widest flex items-center justify-center gap-2 transition-all duration-200 active:scale-[0.99] disabled:opacity-50 cursor-pointer"
            >
              {isAuthenticating ? (
                <>
